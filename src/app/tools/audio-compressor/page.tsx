...
import { ChevronRight, Download, Loader2, Waves, RotateCcw } from "lucide-react";
...
const related = getRelatedTools("audio-compressor");
const MAX_AUDIO_SIZE_BYTES = 100 * 1024 * 1024;
const BITRATES = [64, 96, 128, 192] as const;

type Stage = "idle" | "loading-engine" | "converting" | "error";

export default function AudioCompressorPage() {
  const [file, setFile] = useState<File | null>(null);
  const [bitrate, setBitrate] = useState<(typeof BITRATES)[number]>(128);
  ...
  async function compress() {
    ...
    try {
      ...
      const code = await ffmpeg.exec(["-i", inputName, "-b:a", `${bitrate}k`, "-vn", "output.mp3"]);
      if (code !== 0) throw new Error("Compression failed for this file.");
      ...
      setError(err instanceof Error ? err.message : "Couldn't compress that file.");
      setStage("error");
    } finally {
  ...
  const saved = file && result ? percentSaved(file.size, result.blob.size) : 0;
  ...
  return (
    <main id="main-content">
      <nav aria-label="Breadcrumb" className="mx-auto flex max-w-3xl items-center gap-1.5 px-4 pt-8 text-xs text-muted-foreground sm:px-6">
        ...
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground">Audio Compressor</span>
      </nav>

      <section className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-4 pb-10 pt-8 text-center sm:px-6">
        <h1 className="text-balance text-4xl font-bold tracking-tight sm:text-5xl">Audio Compressor</h1>
        <p className="max-w-xl text-balance text-muted-foreground">
          Re-encode an audio file to a lower bitrate MP3 to shrink it, processed entirely in your
                  ...
                  {BITRATES.map((b) => (
                    <button
                      ...
                      className={`rounded-full border px-4 py-1.5 text-sm font-medium transition-colors ${
                        bitrate === b ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {b} kbps
                    </button>
              ...
              {result && (
                <div className="flex flex-col gap-3 border-t border-border pt-5 sm:flex-row sm:items-center">
                  <p className="flex-1 text-sm text-muted-foreground">
                    {formatBytes(result.blob.size)} · Saved {saved}%
                  </p>
                  <a
                    href={result.url}
