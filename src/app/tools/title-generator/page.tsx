...
const related = getRelatedTools("title-generator");

const FIELDS: GeneratorField[] = [
  { key: "topic", label: "Video topic", placeholder: "What's the video about?", type: "textarea", required: true },
  { key: "tone", label: "Tone (optional)", placeholder: "", type: "select", options: ["Curious", "Bold", "Educational", "Funny", "Urgent"] },
];

export const metadata: Metadata = {
  title: "Title Generator",
  description: "Generate YouTube title concepts for a topic with AI.",
};
  ...
  return (
    <main id="main-content">
      <nav aria-label="Breadcrumb" className="mx-auto flex max-w-3xl items-center gap-1.5 px-4 pt-8 text-xs text-muted-foreground sm:px-6">
        ...
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground">Title Generator</span>
      </nav>

      <section className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-4 pb-10 pt-8 text-center sm:px-6">
        <h1 className="text-balance text-4xl font-bold tracking-tight sm:text-5xl">Title Generator</h1>
        <p className="max-w-xl text-balance text-muted-foreground">
          Generate multiple YouTube title concepts for your video.
        </p>
      </section>

      <section className="mx-auto max-w-3xl px-4 pb-16 sm:px-6">
        <AiGeneratorTool kind="title" fields={FIELDS} submitLabel="Generate Titles" />
      </section>
