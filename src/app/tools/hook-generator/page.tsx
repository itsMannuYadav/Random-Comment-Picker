...
const related = getRelatedTools("hook-generator");

const FIELDS: GeneratorField[] = [
  { key: "topic", label: "Topic", placeholder: "What's the video or post about?", type: "textarea", required: true },
  { key: "tone", label: "Tone (optional)", placeholder: "", type: "select", options: ["Curious", "Bold", "Funny", "Dramatic", "Calm"] },
];

export const metadata: Metadata = {
  title: "Hook Generator",
  description: "Generate opening hooks for a video or post with AI.",
};
  ...
  return (
    <main id="main-content">
      <nav aria-label="Breadcrumb" className="mx-auto flex max-w-3xl items-center gap-1.5 px-4 pt-8 text-xs text-muted-foreground sm:px-6">
        ...
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground">Hook Generator</span>
      </nav>

      <section className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-4 pb-10 pt-8 text-center sm:px-6">
        <h1 className="text-balance text-4xl font-bold tracking-tight sm:text-5xl">Hook Generator</h1>
        <p className="max-w-xl text-balance text-muted-foreground">
          Generate opening hooks for your next video or post.
        </p>
      </section>

      <section className="mx-auto max-w-3xl px-4 pb-16 sm:px-6">
        <AiGeneratorTool kind="hook" fields={FIELDS} submitLabel="Generate Hooks" />
      </section>
